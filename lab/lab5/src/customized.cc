...
extern "C" void customized_conv2d(int8_t *data, int8_t *weights, int *out,
    ...
    int ofmap_W = ifmap_W + padding_L + padding_R - (filter_W-1);
    int ofmap_H = ifmap_H + padding_T + padding_B - (filter_H-1);
    ...
    for (int out_c = 0; out_c < filter_N; out_c += 8) {
        ...
        for (int j = 0; j < padded_h; j += tiled_stride_h) {
            ...
            for (int i = 0; i < padded_w; i += tiled_stride_w) {
                ...
                int tiled_w = std::min(tiled_size, padded_w - i); // width you tiled to transfer ifmap from DRAM to SRAM
                int tiled_h = std::min(tiled_size, padded_h - j); // height you tiled to transfer ifmap from DRAM to SRAM
                int padding_t = j == 0;                  // padding top (1 for padding)
                int padding_l = i == 0;                  // padding left (1 for padding)
                int padding_r = i + tiled_w >= padded_w; // padding right (1 for padding)
                int padding_b = j + tiled_h >= padded_h; // padding bottom (1 for padding)
